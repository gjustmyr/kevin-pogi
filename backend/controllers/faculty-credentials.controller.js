...
exports.createOrUpdateCredentials = async (req, res) => {
  try {
    ...
    if (!education || !educationObtainedWhere || !educationObtainedWhen || !specialization || !subjectsToTeach || !appointmentNature || !status) {
    ...
    if (!credential && (!torFile || !pdsFile || !diplomaFile)) {
      return res.status(400).json({ message: "Please upload all required documents (TOR, PDS, Diploma)" });
    }
    ...
    if (torFile) {
      torPath = path.join(uploadDir, `${faculty.faculty_id}_tor_${Date.now()}${path.extname(torFile.originalname)}`);
      await fs.writeFile(torPath, torFile.buffer);
    }
    if (pdsFile) {
      pdsPath = path.join(uploadDir, `${faculty.faculty_id}_pds_${Date.now()}${path.extname(pdsFile.originalname)}`);
      await fs.writeFile(pdsPath, pdsFile.buffer);
    }
    if (diplomaFile) {
      diplomaPath = path.join(uploadDir, `${faculty.faculty_id}_diploma_${Date.now()}${path.extname(diplomaFile.originalname)}`);
