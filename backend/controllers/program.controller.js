...
exports.createProgram = async (req, res) => {
  try {
    const { program_name, program_acronym, department_id } = req.body;

    // Validate input
    if (!program_name || !program_acronym || !department_id) {
      return res.status(400).json({
        message: "Program name, acronym, and department are required",
      });
    }
    ...
    const existingProgram = await db.Program.findOne({
      where: {
        [Op.or]: [
          ...
          { program_acronym, department_id },
        ],
      },
    });

    if (existingProgram) {
      if (existingProgram.program_name === program_name) {
        return res
          .status(400)
          .json({ message: "Program name already exists in this department" });
      }
      if (existingProgram.program_acronym === program_acronym) {
        return res.status(400).json({
          message: "Program acronym already exists in this department",
        });
      }
    }
...
exports.updateProgram = async (req, res) => {
  try {
    ...
    // Check if new name or acronym conflicts with existing program in the same department
    if (
      ...
      if (existingProgram) {
        if (existingProgram.program_name === program_name) {
          return res
            .status(400)
            .json({
              message: "Program name already exists in this department",
            });
        }
        if (existingProgram.program_acronym === program_acronym) {
          return res
